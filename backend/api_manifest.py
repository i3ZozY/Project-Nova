"""Management of the Project Nova API manifest (free API list)."""
...
def fetch_free_apis_now(content_script_query: str = "") -> str:
    ...
    client = ensure_http_client("Project Nova: FetchFreeApisNow")
    try:
        logger.log("Project Nova: FetchFreeApisNow invoked")
        manifest_text = ""

        try:
            logger.log(f"Project Nova: Fetching manifest from {API_MANIFEST_URL}")
            resp = client.get(API_MANIFEST_URL, follow_redirects=True)
            logger.log(f"Project Nova: Manifest response: status={resp.status_code}")
            resp.raise_for_status()
            manifest_text = resp.text
            logger.log("Project Nova: Fetched manifest from primary URL")
        except Exception as primary_err:
            logger.warn(f"Project Nova: Primary manifest URL failed ({primary_err}), trying proxy...")
            try:
                logger.log(f"Project Nova: Fetching manifest from proxy {API_MANIFEST_PROXY_URL}")
                resp = client.get(
                    API_MANIFEST_PROXY_URL,
                    follow_redirects=True,
                    timeout=HTTP_PROXY_TIMEOUT_SECONDS,
                )
                logger.log(f"Project Nova: Proxy manifest response: status={resp.status_code}")
                resp.raise_for_status()
                manifest_text = resp.text
                logger.log("Project Nova: Fetched manifest from proxy URL")
            except Exception as proxy_err:
                logger.warn(f"Project Nova: Proxy manifest URL also failed: {proxy_err}")
                return json.dumps(
        ...
        logger.warn(f"Project Nova: FetchFreeApisNow failed: {exc}")
...
def load_api_manifest() -> List[Dict[str, Any]]:
    ...
    if normalized and normalized != text:
        try:
            ...
            logger.log("Project Nova: Normalized api.json")
        except Exception:
            pass
        text = normalized
    ...
    try:
        ...
        logger.error(f"Project Nova: Failed to parse api.json: {exc}")
        return []
...
def get_api_list(content_script_query: str = "") -> str:
    ...
    try:
        ...
        logger.error(f"Project Nova: Failed to get API list: {exc}")
