...
def _fetch_fixes_index() -> Optional[Dict]:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.log(f"Project Nova: Fixes index loaded ({len(generic_set)} generic, {len(online_set)} online)")
            return index
        else:
            logger.warn(f"Project Nova: Fixes index fetch returned {resp.status_code}")
...
def check_for_fixes(appid: int) -> str:
    ...
    if index is not None:
        ...
        try:
            ...
            logger.warn(f"Project Nova: Generic fix check failed for {appid}: {exc}")
        ...
        try:
            ...
            logger.warn(f"Project Nova: Online-fix check failed for {appid}: {exc}")
    ...
    try:
        ...
        with zipfile.ZipFile(dest_zip, "r") as archive:
            ...
            if len(top_level_entries) == 1 and appid_folder.rstrip("/") in top_level_entries:
                logger.log(f"Project Nova: Found single folder {appid} in zip, extracting its contents")
                ...
                logger.log(f"Project Nova: Extracting all zip contents to {install_path}")
                for member in archive.namelist():
                    if member.endswith("/"):
                        continue
                ...
                if ini_relative_path:
                    ...
                    if os.path.exists(ini_full_path):
                        ...
                        if updated_contents != contents:
                            ...
                            logger.log(f"Project Nova: Updated unsteam.ini with appid {appid}")
                        else:
                            logger.log("Project Nova: unsteam.ini did not contain <appid> placeholder or was already updated")
                    else:
                        logger.warn(f"Project Nova: Expected unsteam.ini at {ini_full_path} but file not found")
                else:
                    logger.warn("Project Nova: Extracted files do not include unsteam.ini for Online Fix (Unsteam)")
            except Exception as exc:
                logger.warn(f"Project Nova: Failed to update unsteam.ini: {exc}")
        ...
        try:
            ...
            logger.log(f"Project Nova: Appended fix log at {log_file_path} with {len(extracted_files)} files")
        except Exception as exc:
...
def _unfix_game_worker(appid: int, install_path: str, fix_date: str = None):
    try:
        logger.log(f"Project Nova: Starting un-fix for appid {appid}, fix_date={fix_date}")
        log_file_path = os.path.join(install_path, f"projectnova-fix-log-{appid}.log")
        ...
        for file_path in files_to_delete:
            try:
                ...
                if os.path.exists(full_path):
                    os.remove(full_path)
                    deleted_count += 1
                    logger.log(f"Project Nova: Deleted {file_path}")
            except Exception as exc:
                logger.warn(f"Project Nova: Failed to delete {file_path}: {exc}")
        ...
        if remaining_fixes:
            try:
                ...
                logger.log(f"Project Nova: Updated log file, {len(remaining_fixes)} fixes remaining")
            except Exception as exc:
...
def get_installed_fixes() -> str:
    """Scan all Steam library folders for games with Project Nova fix logs."""
    try:
        ...
        try:
            ...
            logger.warn(f"Project Nova: Failed to parse libraryfolders.vdf: {exc}")
