"""Auto-update utilities for the Project Nova backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"Project Nova updated to {new_version}. Please restart Steam."
        return "Project Nova update applied. Please restart Steam."
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        return f"Project Nova updated to {latest_version}. Please restart Steam."
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"Project Nova: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("Project Nova: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"Project Nova: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"Project Nova: CheckForUpdatesNow failed: {exc}")
